...
//        normal = texture(texture_normal, TexCoord).rgb;
    ...
    vec3 ambient = light.ambient * vec3(pow(texColor.rgb, vec3(gamma)));
    //    vec3 ambient = light.ambient * vec3(pow(texture(texture_diffuse1, TexCoord).rgb, vec3(gamma)));
    vec3 diffuse = light.diffuse * diff * vec3(diffuse_color) * vec3(pow(texColor.rgb, vec3(gamma)));
    vec3 specular = light.specular * spec * vec3(specular_color) * vec3(specularTextureColor);
