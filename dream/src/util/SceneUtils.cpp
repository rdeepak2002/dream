...
namespace Dream {
    void SceneUtils::removeMeshReference(Dream::Entity entity, std::string meshGUID, bool recursively) {
        if (entity.hasComponent<Component::MeshComponent>() &&
            entity.getComponent<Component::MeshComponent>().meshType == Component::MeshComponent::MeshType::FROM_FILE &&
            entity.getComponent<Component::MeshComponent>().guid == meshGUID) {
